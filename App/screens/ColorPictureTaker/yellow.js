...
export default function App() {
  ...
  const uploadImageAsync = (pictureuri) =>{
    ...
    console.log('yellow'+val+'.'+fileType)
    data.append('file',{
      uri:pictureuri,
      name:'yellow'+val+'.'+fileType,
      type:'image/'+fileType
    })
    fetch(apiUrl, {
      headers:{
...
return(
    <TouchableOpacity onPress={() => {SendPreview(), navigation.navigate('Gradient')} } style={styles.SendButton}>
      {/* <View style={[styles.closeCross, { transform: [{ rotate: "45deg" }] }]} />
      <View
        style={[styles.closeCross, { transform: [{ rotate: "-45deg" }] }]}
      /> */}
  ...
  <Line x1={w*.142} y1={w*.495} x2={w*.856} y2={w*.495} stroke="darkorange" strokeWidth="5" />
  ...
  <Line x1={w*.142} y1={w*1.195} x2={w*.856} y2={w*1.195} stroke="red" strokeWidth="5" />
</Svg>
  );
