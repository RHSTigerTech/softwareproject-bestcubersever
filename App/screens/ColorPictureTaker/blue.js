...
export default function App() {
  ...
  const uploadImageAsync = (pictureuri) =>{
    ...
    console.log('blue'+val+'.'+fileType)
    data.append('file',{
      uri:pictureuri,
      name:'blue'+val+'.'+fileType,
      type:'image/'+fileType
    })
    fetch(apiUrl, {
      headers:{
...
return(
    <TouchableOpacity onPress={() => {SendPreview(), navigation.navigate('Orange')} } style={styles.SendButton}>
      {/* <View style={[styles.closeCross, { transform: [{ rotate: "45deg" }] }]} />
      <View
        style={[styles.closeCross, { transform: [{ rotate: "-45deg" }] }]}
      /> */}
  ...
  <Line x1={w*.15} y1={w*.495} x2={w*.15} y2={w*1.195} stroke="red" strokeWidth="5"  />
  ...
  <Line x1={w*.85} y1={w*.495} x2={w*.85} y2={w*1.195} stroke="darkorange" strokeWidth="5" />
